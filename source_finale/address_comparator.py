"""Module de comparaison d'adresses avec tolérance aux fautes de frappe"""
...
class AddressComparator:
    """Comparateur d'adresses avec gestion des abréviations et fautes de frappe"""
    ...
    def __init__(self):
        ...
        # Seuils de similarité (strict sauf pour la voie)
    ...
    def compare_numbers(self, num1: str, num2: str) -> float:
        """Compare les numéros d'adresse (doit être exact)"""
        ...
        return 1.0 if numbers1[0] == numbers2[0] else 0.0
    ...
    def compare_postal_codes(self, code1: str, code2: str) -> float:
        """Compare les codes postaux (doit être exact)"""
        ...
        return 1.0 if norm_code1 == norm_code2 else 0.0

    def compare_cities(self, city1: str, city2: str) -> float:
        """Compare les noms de villes (doit être exact après normalisation)"""
        ...
        return 1.0 if norm_city1 == norm_city2 else 0.0
    ...
    def compare_addresses(self, address1: Address, address2: str, logger: Optional[Logger] = None) -> Dict:
        """Compare une adresse structurée avec une adresse en chaîne"""
        if logger:
            logger.log(
                f"{address1['numero']} {address1['voie']}, {address1['code_postal']} {address1['ville']} VS {address2}", 
                "DEBUG"
            )
        ...
        if not parsed_address2:
            return {
                ...
                'details': {'numero': 0.0, 'voie': 0.0, 'code_postal': 0.0, 'ville': 0.0},
        ...
        # Champs stricts doivent être exacts
    ...
    def is_address_match(self, address1: Address, address2: str, logger: Optional[Logger] = None, threshold: float = 0.8) -> bool:
        """Vérifie simplement si deux adresses correspondent"""
